...
def test_user():
    UserService().signup(
        InSignupDTO(
            email="test@test.com", password="testtest1234", re_password="testtest1234"
        )
    )
...
def test_post(test_user):
    ...
    post_id = post_service.create_post(
        title=title, content=content, author_id=test_user.id
    )
...
def test_posts(test_user):
    ...
    for i in range(1, 21):
        title = f"테스트 제목 {i}"
        content = f"테스트 내용 {i}"
        post_service.create_post(title=title, content=content, author_id=test_user.id)
        time.sleep(0.01)
...
def test_another_post(test_another_user):
    ...
    content = f"테스트 다른 내용"
    post_id = post_service.create_post(
        title=title, content=content, author_id=test_another_user.id
    )
    return post_service.get_post(post_id)
