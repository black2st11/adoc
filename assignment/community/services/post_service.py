...
from common.exception_code import (AuthorizationErrorCode, NotFoundErrorCode,
                                   ValidationErrorCode)
...
class PostService:
    ...
    def get_posts(
        ...
        documents = self.collection.get_posts(
            size=size,
            order_by=order_by,
            order=order,
    ...
    def update_post(self, post_id: Any, author_id: int, title: str, content: str):
        ...
        if post.get("author_id") != author_id:
            raise CustomException(
                message=_("해당 작업을 할 권한이 없습니다."),
                status_code=403,
                code=AuthorizationErrorCode,
            )
        return self.collection.update_post(
    ...
    def _validate_data(title: str, content: str):
        if not title:
            raise CustomException(
                message=_("제목을 입력해주세요."),
                status_code=400,
                code=ValidationErrorCode,
            )
        if not content:
            raise CustomException(
                message=_("내용을 입력해주세요."),
                status_code=400,
                code=ValidationErrorCode,
            )
